...
class TooltipPreviews extends HTMLElement {
  constructor({ image, content, subscript, link } = {}) {
    ...
    ${image ? `<div class="tooltip--previews__image-wrapper"${image.h ? ` style="--h: ${image.h}px;"` : ''}><img class="tooltip--previews__image" src="${image.src}" alt="preview" loading="eager" style="${image.shift ? `--shift: ${image.shift};` : ''}${image.opacity ? ` --image-opacity: ${image.opacity};` : ''}"${image.blur ? ` data-blur="${image.blur}"` : ''}${image.key ? ` data-key-image="${image.key}"` : ''}><button type="button" class="tooltip--previews__image__button-toggle-fullres-wrapper material-icons" title="${nk.locale.get('buttonLabels.fullres')}" title-key="buttonLabels.fullres">fullscreen</button></div>` : ''}
